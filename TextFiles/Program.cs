...
static void WriteToFile(string filename)
{
    ...
    if (File.Exists(filename)) fileStr = File.AppendText(filename);
    else fileStr = File.CreateText(filename);

    foreach (string line in contents) fileStr.WriteLine(line);
...
static List<string> FindStationsByFilter(string filter, string filename)
{
    ...
    while ((line = fileStr.ReadLine()) != null) if (line.ToLowerInvariant().Contains(filter)) results.Add(line.Split(", ")[0]);
...
static List<string> FindStationsByException(string filter, string filename)
{
    StreamReader fileStr = File.OpenText(filename);
    ...
    {
        valid = true;
        string station_name = line.Split(", ")[0];

        string lower_station_name = station_name.ToLowerInvariant();

        foreach (char c in filter) if (lower_station_name.Contains(c)) valid = false;

        if (valid) results.Add(station_name);
    }
...
Debug.Assert(FindStationsByException("Mackerel", "stations.txt").SequenceEqual(["St. John's Wood"]));
Debug.Assert(FindStationsByException("Piranha", "stations.txt").SequenceEqual(["Stockwell"]));
Debug.Assert(FindStationsByException("Sturgeon", "stations.txt").SequenceEqual(["Balham", "Blackwall"]));
Debug.Assert(FindStationsByException("Bacteria", "stations.txt").SequenceEqual([]));
...
Debug.Assert(FindStationsByRepeat("stations.txt").SequenceEqual(["Charing Cross", "Clapham Common", "Golders Green", "Seven Sisters", "Sloane Square"]));
